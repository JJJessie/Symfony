...
默认情况下，Heroku 会开始一个 Apache 网页服务器和 PHP 一并来服务应用程序。然而，有两个特殊情况适用于 Symfony 应用程序：

1.	文件根是在 **web/** 目录中，而不是在用应用程序的根目录中。
2.	
2.	Composer **bin-dir**，即供应商的二进制文件（因此 Heroku 自身的引导脚本）放置的地方，是 **bin/**，不是默认的 **vendor/bin**。
...
在部署中，Heroku 运行 **composer install --no-dev** 来安装应用程序所需要的所有依赖。然而，一般在 **composer.json** [安装后命令](https://getcomposer.org/doc/articles/scripts.md)，例如：安装资产或者清除（或预热）缓存，在默认情况下运行使用 Symfony的 **dev** 环境。
...
注意在 **require-dev** 区段 **composer.json** 列出的依赖在Heroku中部署的过程中从不被安装。如果您的 Symfony 环境依赖这些包的话可能会引发问题。解决方案就是从 **require-dev** 移除这些包到 **require** 区段。

### 启动 Heroku 的代码

下一步，终于到了在 Heroku 部署您的应用程序的时间。如果您做这个是第一次的话，您将会看到像如下的信息：
...
这样就好了！如果您现在打开您的浏览器，要不就手动指到 **heroku create** 给您的 URL，或者也可以使用 Heroku Toolbelt，应用程序将会响应：
...
根据下一次部署，Heroku 使用 Node.js 构建包来编译应用程序并且安装您的 npm 包。另一方面，**composer.json** 现在被忽略。用两个构建包编译您的应用程序的话，Node.js *和* PHP，您可以使用一个特殊的[多样构建](https://github.com/ddollar/heroku-buildpack-multi)。为了覆盖构建包自动侦测，您需要明确地设置构建包 URL：
...
有了下一次部署，您可以从双方构建包获益。此设置也启动您的 Heroku 环境，充分使用节点，基于自动构建工具像 [Grunt](http://gruntjs.com/) 或者 [gulp](http://gulpjs.com/)。
